...
function convertStr(stringVal) {
    if (Math.floor(stringVal / 1000) > 1) {
        return Math.floor(stringVal / 1000) + "." + Math.round((stringVal / 1000 - Math.floor(stringVal / 1000)) * 10) + "K";
    }
    return stringVal
}
...
export default function User(props) {
    ...
    return (
        <>
            <div className="container-main-user-picture">
                <img src={props.urlPhoto} />
            </div>
            <div className="container-main-user-initials">
                <label className="container-main-user-initials-name">{props.name}</label>
                <a className="container-main-user-initials-login" href={props.urlUser} rel="noreferrer"
                   target="_blank">  {props.login} </a>
            </div>

            <div className="container-main-user-info">
                <div className="container-main-user-info-followers">
                    ...
                    <label> {convertStr(props.followers)} followers</label>
                </div>
                <div className="container-main-user-info-following">
                    <svg width="25" height="25" viewBox="0 0 25 25" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path fillRule="evenodd" clipRule="evenodd"
                    ...
                    <div>{convertStr(props.following)} following</div>
                </div>
            </div>
        </>
    )
